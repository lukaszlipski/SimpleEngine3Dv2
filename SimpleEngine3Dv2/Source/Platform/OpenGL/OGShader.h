...
namespace SE3D2
{
	...
	class OGComputeShaderPolicy
	{
	public:
		inline uint32 CreateShader() { return glCreateShader(GL_COMPUTE_SHADER); }
	...
	bool OGShader<T>::SetStructuredBuffer(const std::string& name, StructuredBuffer* sb)
	{
		for (auto& Buff : mStructuredBuffers)
		{
			if (Buff.mName == name)
			{
				if (sb)
				{
					glBindBufferBase(GL_SHADER_STORAGE_BUFFER, Buff.mSlot, static_cast<OGStructuredBuffer*>(sb)->GetBuffer());
				}
				else
				{
					glBindBufferBase(GL_SHADER_STORAGE_BUFFER, Buff.mSlot, 0);
				}
				return true;
			}
		}
	...
	bool OGShader<T>::CollectResources(const std::string& source)
	{
		...
		// Get ssbo
		int32 NumBlocks;
		glGetProgramInterfaceiv(mProgram, GL_SHADER_STORAGE_BLOCK, GL_ACTIVE_RESOURCES, &NumBlocks);
		...
		{
			GLint ValuesBlock[3];
			glGetProgramResourceiv(mProgram, GL_SHADER_STORAGE_BLOCK, 0, 3, PropertiesBlock, 3, 0, ValuesBlock);
			char* Name = (char*)alloca(ValuesBlock[0]);
			glGetProgramResourceName(mProgram, GL_SHADER_STORAGE_BLOCK, 0, ValuesBlock[0], &ValuesBlock[0], Name);
			
			mStructuredBuffers.push_back({ Name , static_cast<uint32>(ValuesBlock[2]), static_cast<uint32>(ValuesBlock[1]) });
	...
	using OGComputeShader = OGShader<OGComputeShaderPolicy>;
